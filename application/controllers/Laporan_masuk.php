...
class Laporan_masuk extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->session->set_flashdata('title', 'Laporan Barang Masuk | HELLO PRINT');
    ...
    public function index()
    {
        ...
        $this->template->load('template', 'laporan_masuk/laporan');
    }

    public function ajax_list()
    {
        $list = $this->Laporan_masuk_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $sum = 0;
        foreach ($list as $customers) {
            ...
            $row[] = '<a class="btn btn-sm btn-primary" href="' . base_url('laporan_masuk/print/' . $customers->no_po) . '" title="Edit" target="_blank">Print</a>';
            $data[] = $row;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Laporan_masuk_model->count_all(),
            "recordsFiltered" => $this->Laporan_masuk_model->count_filtered(),
            "data" => $data,
            // "sum"             => $sum,
        );
    ...
    public function read($no_po)
    {
        ...
        $this->template->load('template', 'laporan_masuk/laporan_read', $data);
    ...
    public function pdf()
    {
        ...
        $nourut = 1;
        foreach ($this->Laporan_masuk_model->excel($tanggal_a, $tanggal_b) as $data) {
    ...
    public function report()
    {
        ...
        $this->pdfgenerator->load_view('laporan_masuk/laporan_pdf', $data);
    }
    ...
    function print($no_po) {
        ...
        $this->pdfgenerator->setPaper('A4', 'landscape');
        $this->pdfgenerator->filename = "laporan.pdf";
        $this->pdfgenerator->load_view('laporan_masuk/laporan_print', $data);
    }
