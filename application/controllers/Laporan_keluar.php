...
class Laporan_keluar extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->load('template', 'laporan_keluar/laporan');
    }

    public function ajax_list()
    {
        $list = $this->Laporan_keluar_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $sum = 0;
        foreach ($list as $customers) {
            ...
            $row[] = '<a class="btn btn-sm btn-primary" href="' . base_url('laporan_keluar/print/' . $customers->notrans) . '" title="Edit" target="_blank">Print</a>';
            $data[] = $row;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Laporan_keluar_model->count_all(),
            "recordsFiltered" => $this->Laporan_keluar_model->count_filtered(),
            "data" => $data,
            // "sum"             => $sum,
        );
    ...
    public function read($notrans)
    {
        ...
        $this->template->load('template', 'laporan_keluar/laporan_read', $data);
    ...
    public function pdf()
    {
        ...
        $nourut = 1;
        foreach ($this->Laporan_keluar_model->excel($tanggal_a, $tanggal_b) as $data) {
    ...
    public function report()
    {
        ...
        $this->pdfgenerator->load_view('laporan_keluar/laporan_pdf', $data);
    }
    ...
    function print($notrans) {
        ...
        $this->pdfgenerator->setPaper('A4', 'landscape');
        $this->pdfgenerator->filename = "laporan.pdf";
        $this->pdfgenerator->load_view('laporan_keluar/laporan_print', $data);
    }
